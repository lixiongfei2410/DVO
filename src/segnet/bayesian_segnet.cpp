...
BayesianSegNet::BayesianSegNet(const BayesianSegNetParams &params)
    ...
    this->checkConfig();  //判断输入的param是否为空,params为结构体,初始化配置
...
//每个批次需要处理的数据 指针 input_batch
void BayesianSegNet::wrapInputLayer(
  std::vector<std::vector<cv::Mat>> &input_batch) {
    int batch = this->input_layer->shape(0);//3
    int channels = this->input_layer->shape(1);//3
    int height = this->input_layer->shape(2);//352
    int width = this->input_layer->shape(3);//1024
